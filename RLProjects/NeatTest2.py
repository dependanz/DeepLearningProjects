from RLProjects.NEAT.NEAT import feedforward,connection_mutation,node_mutation
...
elite_size = 10
selection_prob = 0.35
weight_shift_prob = 0.3
shift_radius = 0.01
capability_thresh = 0.2
...
test_x, test_y = xor_random_tests(-1)
...
while 1:
    ...
    if(choice[0] == 'c'):
        ...
        generations += 1
        test_population.next_generation(test_x, test_y, fitness_function,
                                        connection_mutation_rate=connection_mutation_rate,
                                        node_mutation_rate=node_mutation_rate, elite_size=elite_size,
                                        selection_prob=selection_prob, weight_shift_prob=weight_shift_prob,
                                        shift_radius=shift_radius, c_thresh=capability_thresh)
        test_population.evaluate_population(test_x, test_y, fitness_function)

        for i in range(10):
            g = test_population.genomes[i]
            # print(f"genome {i+1} fitness: {g[1]}")
            g[0].visualize("Geneome " + str(i + 1), 'Genome_' + str(i + 1),fitness=g[1])
    elif(choice == "s"):
        ...
        g[0].visualize("Visualization of Geneome " + str(choice + 1), 'V_Genome_' + str(choice + 1),fitness=g[1])
    elif(choice == "p"):
        print(f"Innovation Nodes: {len(test_population.innovation_manager.innovation_numbers)}")
    elif(choice == "m"):
        choice = input(f"Connection [c] or Node [n]? -> ")
        if(choice == "c"):
            choice = int(input(f"Which Genome to mutate? [0 - {len(test_population.get_genomes()) - 1}]\n-> "))
            connection_mutation(test_population.genomes[choice][0],test_population.innovation_manager)
            test_population.genomes[choice][0].visualize("Geneome " + str(choice + 1), 'Genome_' + str(choice + 1),fitness=test_population.genomes[choice][1])
        elif(choice == "n"):
            choice = int(input(f"Which Genome to mutate? [0 - {len(test_population.get_genomes()) - 1}]\n-> "))
            node_mutation(test_population.genomes[choice][0], test_population.innovation_manager)
            test_population.genomes[choice][0].visualize("Geneome " + str(choice + 1), 'Genome_' + str(choice + 1),fitness=test_population.genomes[choice][1])
